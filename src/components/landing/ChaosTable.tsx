    ...
    <section className="section-container border-t-[3px] border-dashed border-muted">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-4 mb-3">
          <div
            className="text-center py-2 font-heading text-base text-ink/50 bg-muted border-2 border-ink/20"
            style={{ borderRadius: '180px 45px 200px 35px / 40px 190px 30px 170px' }}
          >
            ✉️ The Old Way (Email Chaos)
          </div>
          <div
            className="text-center py-2 font-heading text-base text-ink bg-ink text-paper border-2 border-ink"
        ...
        <div className="flex flex-col gap-3">
          {ROWS.map((row, i) => (
            ...
            >
              {/* Bad side */}
              <div
                className="flex items-start gap-2.5 px-4 py-3 bg-[#f5f0ea] border-2 border-ink/20"
                style={{ borderRadius: '220px 30px 240px 20px / 25px 230px 20px 215px' }}
              >
                <div className="w-5 h-5 flex-shrink-0 rounded-full bg-red-100 border-2 border-red-300 flex items-center justify-center mt-0.5">
                  <X size={10} strokeWidth={3} className="text-red-500" />
                </div>
                <span className="font-body text-sm text-ink/55 line-through">{row.bad}</span>
              </div>

              {/* Good side */}
              <div
                className="flex items-start gap-2.5 px-4 py-3 bg-[#f0fdf4] border-2 border-green-300"
                style={{
                  borderRadius: '220px 30px 240px 20px / 25px 230px 20px 215px',
                  boxShadow: '3px 3px 0 0 #2d2d2d',
                }}
              >
                <div className="w-5 h-5 flex-shrink-0 rounded-full bg-green-100 border-2 border-green-400 flex items-center justify-center mt-0.5">
                  <Check size={10} strokeWidth={3} className="text-green-600" />
                </div>
                <span className="font-body text-sm text-ink font-bold">{row.good}</span>
              </div>
            </div>
          ))}
        </div>
